...
import sqlite3, json, hmac, hashlib, os, uuid
...
def validate_init_data(init_data: str) -> dict:
    if not init_data: raise HTTPException(status_code=401, detail="No initData")
    try:
        params = {}
    ...
    except Exception as e: raise HTTPException(status_code=401, detail=f"Invalid: {str(e)}")
...
async def join_wallet(req: JoinWalletReq):
    ...
    if not c.fetchone(): raise HTTPException(status_code=404, detail="Wallet not found")
...
async def get_balances(wallet_id: str, x_init_data: str = Header(None)):
    ...
    if not c.fetchone(): raise HTTPException(status_code=403, detail="Not member")
...
async def add_expense(wallet_id: str, req: ExpenseReq, x_init_data: str = Header(None)):
    user_data = validate_init_data(x_init_data)
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT telegram_id FROM wallet_users WHERE wallet_id = ? AND telegram_id = ?', (wallet_id, user_data.get('id')))
    ...
    result = c.fetchone()
    if not result or result[1] < req.amount: raise HTTPException(status_code=400, detail="Insufficient funds")
    acc_id = result[0]
...
async def transfer(wallet_id: str, req: TransferReq, x_init_data: str = Header(None)):
    user_data = validate_init_data(x_init_data)
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT telegram_id FROM wallet_users WHERE wallet_id = ? AND telegram_id = ?', (wallet_id, user_data.get('id')))
    ...
    if not from_result or from_result[1] < req.amount: raise HTTPException(status_code=400, detail="Insufficient funds")
    c.execute('SELECT id FROM accounts WHERE wallet_id = ? AND type = ?', (wallet_id, req.to_type))
    to_result = c.fetchone()
    if not to_result: raise HTTPException(status_code=400, detail="Invalid target")
...
async def get_operations(wallet_id: str, limit: int = 100, x_init_data: str = Header(None)):
    user_data = validate_init_data(x_init_data)
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT telegram_id FROM wallet_users WHERE wallet_id = ? AND telegram_id = ?', (wallet_id, user_data.get('id')))
...
async def get_wallet_users(wallet_id: str, x_init_data: str = Header(None)):
    user_data = validate_init_data(x_init_data)
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
